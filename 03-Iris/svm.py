...
# 1.读取数据集
path = r'G:\Kaggle\03 Iris\Iris.data'
data = np.loadtxt(path, dtype=float, delimiter=',', converters={4: Iris_label})
# converters={4:Iris_label}中“4”指的是第5列：将第5列的str转化为label(number)
# converters：将数据列与转换函数进行映射的字典。例
# converters = {4: Iris_label}
# 含义是将第5列的数据对应转换函数进行转换。
# print(data.shape)

# 2.划分数据与标签
x, y = np.split(data, indices_or_sections=(4,), axis=1)  # x为数据，y为标签,axis 1水平0垂直
x = x[:, 0:2]
train_data, test_data, train_label, test_label = train_test_split(x, y, random_state=1, train_size=0.6,
                                                                  test_size=0.4)  # sklearn.model_selection.随机划分
